...
//    public TransactionSignature signedTranSig = null;
//    public ECKey.ECDSASignature signedSig = null;
//    public boolean MPCSign = true;
//    public Sha256Hash hash2MPC = null;
//    public String bosHex = null;
    public boolean innerSign = false;

    public Sha256Hash[] hashArray = null; //new Sha256Hash[getInputs().size()];
    public int hashedInt = 0;
    ...
    public TransactionSignature calculateWitnessSignature(
        ...
        if (innerSign == false && hashedInt < getInputs().size()){
            hashArray[inputIndex] = hash;
            hashedInt++;

            return null;
        }
//        else{
//            ECKey.ECDSASignature sig = new ECKey.ECDSASignature(new BigInteger(strR, 16),
//                    new BigInteger(strS, 16)).toCanonicalised();
//
//            return new TransactionSignature(sig, hashType, anyoneCanPay);
//        }
        ...
        if (key.hasPrivKey()){
            ...
            System.out.println("Send to ZenGoX to sign..... : " + hash);

            if (innerSign) {
                //local sign.   initial only public key.
                NetworkParameters n = NetworkParameters.fromID(NetworkParameters.ID_REGTEST);
                DumpedPrivateKey dumpedPrivateKey = DumpedPrivateKey.fromBase58(n, "cNznM2yaxofFPyuN7c6FX4ay7qrR3PyQLgj5WK2hsvsSGavtCYfT");
                key = dumpedPrivateKey.getKey();
                ECKey.ECDSASignature signedSig = key.sign(hash);
                System.out.println("c/p R Biginteger: " + signedSig.r);
                System.out.println("c/p S Biginteger: " + signedSig.s);
                System.out.println("c/p R Hex: " + signedSig.r.toString(16));
                System.out.println("c/p S Hex: " + signedSig.s.toString(16));
            }

/////////////////////works above. but something wrong below....................


            System.out.println("Please copy/paste ZenGo signature here: ..............");
            ...
            strR = sc.nextLine();
            System.out.println("You have entered: " + strR);
            System.out.println("S:");
            sc = new Scanner(System.in);
            strS = sc.nextLine();
            System.out.println("You have entered: " + strS);

            if (strR == null || strS == null || strR.length() < 5 || strS.length() < 5) {
                System.out.println("R or S is null or too short");

//        [
//                "r","2e1348c023b468bf653ca9d44bbc97e90ab7ddbc379102bdffcba06188a46470",
//                "s","3ef3cb49968e87fec98f810ab97e1aac869364335f36926e6d423362a8d9cb16"
//                "v", 1
//        ]
//        ECKey.ECDSASignature sig = new ECKey.ECDSASignature(new BigInteger("2e1348c023b468bf653ca9d44bbc97e90ab7ddbc379102bdffcba06188a46470", 16),
//                new BigInteger("3ef3cb49968e87fec98f810ab97e1aac869364335f36926e6d423362a8d9cb16", 16));
//        return new TransactionSignature(sig, hashType, anyoneCanPay);

//                if (hash2MPC == null) {
//                    System.out.println("new hash created = " + hash);
//                    hash2MPC = hash;
//                } else {
//                    hash = hash2MPC;
//                    System.out.println("hash has initial value, now using it = " + hash2MPC);
//                }
//                if (signedSig == null) {
//                    if (MPCSign) {
//                        return null;
//                    } else {
//                        System.out.println("GENERATING sig.......................................");
//                        signedSig = key.sign(hash);
//                        return new TransactionSignature(signedSig, hashType, anyoneCanPay);
//                    }
//
//                } else {
//                    System.out.println("USING OUTSIDE sig..................comparing test....");
//
//                    ECKey.ECDSASignature sig = new ECKey.ECDSASignature(new BigInteger(signedSig.r.toString(16), 16),
//                            new BigInteger(signedSig.s.toString(16), 16)).toCanonicalised();
//                    System.out.println("comparing R Biginteger: " + signedSig.r);
//                    System.out.println("comparing S Biginteger: " + signedSig.s);
//
//
//
//                    return new TransactionSignature(signedSig, hashType, anyoneCanPay);
//                }
                ...
                System.out.println("Using user inputted R, S signature:))))))))))))");
                ECKey.ECDSASignature sig = new ECKey.ECDSASignature(new BigInteger(strR, 16),
                        new BigInteger(strS, 16)).toCanonicalised();

                System.out.println("comparing manual input  R Biginteger: " + sig.r);
                System.out.println("comparing manual input  S Biginteger: " + sig.s);
                System.out.println("comparing manual input r = " + sig.r.toString(16));
                System.out.println("comparing manual input s = " + sig.s.toString(16));
...
//                signedSig = sig;
    ...
    public TransactionSignature calculateWitnessSignature(
            ...
            SigHash hashType,
            boolean anyoneCanPay) {
        System.out.println("calculateWitnessSignature info: " + inputIndex + "\n\t"
                + key +  "\n\t"
    ...
    public synchronized Sha256Hash hashForWitnessSignature(
        ...
        System.out.println("\n\n before twice hash (saved) = " + Hex.toHexString(bos.toByteArray()));
//        if (bosHex == null)
//            bosHex = Hex.toHexString(bos.toByteArray());
//        else
//        {
//            System.out.println("Using outside bosHex: " + bosHex);
//            return Sha256Hash.twiceOf(Hex.decode(bosHex));
//        }

        Sha256Hash th = Sha256Hash.twiceOf(bos.toByteArray());
        System.out.println("\n\n after twice hash = " + th);
