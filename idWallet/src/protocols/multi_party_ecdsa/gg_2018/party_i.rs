...
impl Keys {
    ...
    pub fn createFromBigInt(n: &BigInt, index: usize) ->Self{
        println!("createFromBigInt ...");

        // println!("working on u ...");
        let i : u64 = index as u64;
        let bi : BigInt = BigInt::from(i);
 
        let u: FE = ECScalar::from(&n.add(&bi));
        //let u: FE = ECScalar::from(&n);


        // println!("working on y...");
        let y = &ECPoint::generator() * &u;
        // println!("working on keypair ...");
    ...
    pub fn createFromHex(s: &String, index: usize) ->Self{
        println!("createFromHex ...");

        // println!("working on u ...");
        let bi = BigInt::from_hex(s).unwrap();

        let u: FE = ECScalar::from(&bi);
        // println!("working on y...");
        let y = &ECPoint::generator() * &u;
        // println!("working on keypair ...");
        //let (ek, dk) = Paillier::keypair_safe_primes().keys();
