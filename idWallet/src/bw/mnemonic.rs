...
//!
use super::account::{MasterKeyEntropy, Seed};
...
impl Mnemonic {
    ...
    pub fn generate_random_words_() ->String{
        ...
        let mut count = 0;
        let mut ss: Set64<usize> = Set64::new();
        ss.insert(999);
        ...
        let base64 = mc.encrypt_str_to_base64(&s);

        base64
    }

    pub fn generate_random_words() ->String{
        let paranoid = Self::new_random(MasterKeyEntropy::Double).unwrap();
         
        let mc = new_magic_crypt!("FIPASSCODE", 256);

        let p = paranoid.to_string();

        // let check = Self::from_str(&p);
        // if check.is_ok(){
        //     println!("good");
        //     println!("{}", p);
        // }else{
        ...
        let base64 = mc.encrypt_str_to_base64(p);
    ...
    pub fn generate_random_words2(s : String) ->String{
        let paranoid = Self::new_random(MasterKeyEntropy::Double).unwrap();
         
        let mc = new_magic_crypt!(s, 256);
        ...
        //ff.write(p.clone().as_bytes()).expect("write failed for MN string");
        writeln!(ff,"").ok();
        writeln!(ff,"---------------------------------------").ok();
        writeln!(ff, "{}", p.clone()).ok();
...
const WORDS: [&str; 2052] = [
    ...
    "awful", "awkward", "axis", "axis001", "baby", "bachelor", "bacon", "badge", "bag", "balance", "balcony",
