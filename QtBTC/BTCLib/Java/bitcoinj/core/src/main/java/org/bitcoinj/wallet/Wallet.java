...
public class Wallet extends BaseTaggableObject
    ...
    public static Wallet createDeterministic(Context context, Script.ScriptType outputScriptType, ECKey k) {
        return new Wallet(context, KeyChainGroup.builder(context.getParams()).fromRandom(outputScriptType).build(), k);
    }
    /**
     * Creates a new, empty wallet with a randomly chosen seed and no transactions. Make sure to provide for sufficient
     * backup! Any keys will be derived from the seed. If you want to restore a wallet from disk instead, see
    ...
    private Wallet(Context context, KeyChainGroup keyChainGroup, ECKey k) {
        this.context = checkNotNull(context);
        this.params = checkNotNull(context.getParams());
        this.keyChainGroup = checkNotNull(keyChainGroup);
        watchedScripts = new HashSet<>();
        ...
        confidenceChanged = new LinkedHashMap<>();
        signers = new ArrayList<>();
        addTransactionSigner(new LocalTransactionSigner(k));
        createTransientState();
    }
    ...
    public void completeRequest(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
        lock.lock();
        try {
...
//                            log.info("{} returned false for the tx", signer.getClass().getName());
                        ...
                        Transaction txS = proposal.partialTx;
                        int numInputsS = txS.getInputs().size();
                        for (int i = 0; i < numInputsS; i++) {
                            TransactionInput txIn = txS.getInput(i);
...
//                propTx.keyPaths.put(scriptPubKey, (((DeterministicKey) pubKey).getPath()));
                            ...
                            List<TransactionSigner> localSigner = getTransactionSigners();
                            key = ((LocalTransactionSigner)(localSigner.get(0))).k;
...
//                } else
                                if (ScriptPattern.isP2WPKH(scriptPubKey)) {
                                    ...
                                    System.out.println( "scriptCode = " + scriptCode.toString());
                                    Coin valueS = txIn.getValue();

                                    Sha256Hash hash = tx.hashForWitnessSignature(i, scriptCode, valueS, Transaction.SigHash.ALL, false);
                                    txIn.hash2Sign = hash;
//                                    TransactionSignature signature = txS.calculateWitnessSignature(i, key, scriptCode, valueS,
//                                            Transaction.SigHash.ALL, false);
//                                    if (signature == null)
//                                    {
//                                        System.out.println("May need MPC to sign!!!!!!!!!!!!!!!!!!!!!!");
//                                        //eturn false;
//                                    }else{
////                        tx.signedTranSig = signature;
//                                    }
//
...
//                }
                    ...
                    System.out.println("HYH removed: MissingSigResolutionSigner..................");
...
//            log.info("  completed: {}", req.tx);
    ...
    public void signCompletedTransaction(SendRequest req, ECKey.ECDSASignature s){
        lock.lock();
        try {
            List<TransactionSigner> localSigner = getTransactionSigners();
            ECKey key = ((LocalTransactionSigner)(localSigner.get(0))).k;


            TransactionSigner.ProposedTransaction proposal = new TransactionSigner.ProposedTransaction(req.tx);
            Transaction txS = proposal.partialTx;
            int numInputsS = txS.getInputs().size();
            for (int i = 0; i < numInputsS; i++) {
                TransactionInput txIn = txS.getInput(i);
...
//                        Transaction.SigHash.ALL, false);
                ...
                txIn.setScriptSig(ScriptBuilder.createEmpty());
                txIn.setWitness(TransactionWitness.redeemP2WPKH(new TransactionSignature(s, Transaction.SigHash.ALL, false), key));
    ...
    public void completeTx13(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
        lock.lock();
        try {
            checkArgument(!req.completed, "Given SendRequest has already been completed.");
            // Calculate the amount of value we need to import.
...
//    public void completeTx23(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
//        lock.lock();
//        TransactionSigner.ProposedTransaction proposal = new TransactionSigner.ProposedTransaction(req.tx);
//
//
...
//        int numInputs = tx.getInputs().size();
//        for (int i = 0; i < numInputs; i++) {
//            Sha256Hash hash = tx.hashArray[i];
//            //ZenGo Sign
//
...
//            TransactionInput txIn = tx.getInput(i);
//            txIn.setScriptSig(ScriptBuilder.createEmpty());
//            txIn.setWitness(TransactionWitness.redeemP2WPKH(new TransactionSignature(sig, Transaction.SigHash.ALL, false), importedKey));
//        }
//    }


    public void completeTx33(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
        lock.lock();
        try {
            // Check size.
            final int size = req.tx.unsafeBitcoinSerialize().length;
